...
func on_timer_timeout():
	...
	if target_enemies.size() > 0:
		for i in level:
			...
			var sword_spawn_position = Global.get_player_global_pos()
			...
			var enemy_direction = sword_spawn_position - Global.get_player_global_pos()
...
func on_ability_upgrade_added(upgrade: AbilityUpgrade, current_upgrades: Dictionary) -> void:
	if upgrade.id == id:
		...
		sword_scale = sword_scale * (1 + Global.get_player_modifier_value(Modifiers.SIZE))
